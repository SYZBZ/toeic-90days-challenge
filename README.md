...
- 閱讀 preset：`1 題 / 1 分鐘`、`10 題 / 5 分鐘`、`20 題 / 10 分鐘`
- 聽力 Part 1/2 preset：`1 題 / 1 分鐘`、`10 題 / 5 分鐘`、`20 題 / 10 分鐘`
- 聽力 Part 3/4 preset：`3 題 / 2 分鐘`、`9 題 / 5 分鐘`、`18 題 / 10 分鐘`（3 題一組）
- 倒數計時與自動交卷
...
- PWA 離線可用
- Sidebar 可折疊（桌面）
- Part 1 使用本地授權圖片庫（`public/part1-images`），Gemini 依圖片 metadata 出題
- 聽力前端流程：Gemini 產題 -> Google Cloud TTS 即時合成 -> 瀏覽器內嵌音訊，不需 Firebase Storage
...
# 本機開發可填；公開部署不建議用 build-time VITE API keys
VITE_GEMINI_API_KEY=
VITE_TTS_API_KEY=
VITE_VERTEX_AI_KEY=
```
...
- `VITE_VERTEX_AI_KEY`：選填，僅作 Part 1 圖片 fallback；預設使用本地圖片庫

安全提醒：Vite 會把 `VITE_*` 變數打包進前端 JavaScript。若部署到公開 GitHub Pages，請不要在 GitHub Secrets/Variables 注入真實 AI API Key；建議登入後到「設定」頁儲存個人 API Key，或只在本機 `.env` 使用。
...
- `users/{uid}/question_pool/{poolDocId}`
  ...
  - 聽力欄位：`imageUrl`, `imageSource`, `transcript`
...
- 公開 GitHub Pages 版本不要用 build-time `VITE_*` 注入 AI API Key；登入後於設定頁填入個人 key 較不容易直接暴露在公開 bundle。
- Firebase Storage 不再是必要服務；部署 Firestore rules/indexes 即可。
- Part 1 圖片授權資訊保存在 `public/part1-images/ATTRIBUTION.json`。
